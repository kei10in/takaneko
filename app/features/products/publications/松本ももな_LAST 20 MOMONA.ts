import { Publication } from "../product";

export const 松本ももな_LAST_20_MOMONA: Publication = {
  slug: "松本ももな LAST 20 MOMONA",
  name: "松本ももな LAST 20 MOMONA",
  date: "2023-12-14",
  kind: "books",
  publisher: "LARME",
  listPrice: 727,
  priceWithTax: 800,
  code: [],
  url: "https://www.amazon.co.jp/dp/B0CQDB23C9",
  coverImages: [
    {
      path: "/publications/2023/2023-12-14_松本ももな_LAST 20 MOMONA.jpg",
      ref: "https://x.com/takanenofficial/status/1736340535319666896",
    },
  ],
  featuredMembers: ["松本ももな"],
  officialTwitter: "https://x.com/takanenofficial/status/1736340535319666896",
  links: [],
};
