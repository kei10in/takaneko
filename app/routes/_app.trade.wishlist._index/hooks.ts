...
import { drawWithList } from "~/features/trade/drawTradeItemList";
...
export const usePhotoWishListImages = (
    ...
    () =>
      wishList.flatMap((x) =>
        x.tradingItemDetails.map((x) => ({
          ...
          status: tradeStateToImageSrc(x.status) ?? "/求.svg",
          name: x.item.name,
          id: x.item.id,
  ...
  const { data, isLoading } = useSWR([`photo-wish-list`, items], async ([_, items]) => {
    const chunkedItems = [];
    ...
    return await Promise.all(
      chunkedItems.map(async (chunk) => {
        const blob = await drawWithList(chunk, "ほしい生写真");
        return {
          blob,
...
export const useMiniPhotoCardWishListImages = (
  ...
  const { data, isLoading } = useSWR([`mini-photo-wish-list`, items], async ([_, items]) => {
    const chunkedItems = [];
    for (let i = 0; i < items.length; i += 30) {
      chunkedItems.push(items.slice(i, i + 30));
    }
    ...
    return await Promise.all(
      chunkedItems.map(async (chunk) => {
        const blob = await drawWithList(chunk, "ほしいミニフォトカード");
        return {
          blob,
          objectURL: URL.createObjectURL(blob),
        };
