...
import { extractPhotoImages } from "./extractPhotoImages.ts";
...
describe("extractPhotoImages", () => {
  it("writes detected photos as numbered WebP files under an extensionless directory", async () => {
    const temporaryDirectory = await mkdtemp(path.join(os.tmpdir(), "takaneko-photo-"));
    temporaryDirectories.push(temporaryDirectory);
    const inputPath = path.join(temporaryDirectory, "sample.input.webp");
    await copyFile(path.resolve("scripts/lib/test-fixture/photo-sample.4.webp"), inputPath);
    ...
    expect(firstImage).toMatchObject({ format: "webp", width: 34, height: 48 });
  });
});
