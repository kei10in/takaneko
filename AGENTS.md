...
- イベントの追加や更新はは `main` で作業します。
- 複数コミットが必要な作業は topic branch を作成して作業します。
- Topic branch は `main` ブランチから作成し、説明的な名前を付けます。
- コミットは細かくし、意味のある単位で行います。
...
12. 手続的なコードよりも宣言的なコードを優先する。手続的なコードは本質的に手続であるものだけにする。
13. 上位レイヤーで宣言的に記述できるようにするために、下位のレイヤーを実装する。
